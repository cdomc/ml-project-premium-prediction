...
def calculate_normalized_risk(medical_history):
    ...
    normalize_risk_score = (total_risk_score - min_score) / (max_score - min_score)

    return normalize_risk_score


def preprocess_input(input_dict):
    expected_cols = [
        'age', 'number_of_dependants', 'income_lakhs', 'insurance_plan', 'genetical_risk', 'normalize_risk_score',
...
def handle_scaling(age, df):
    if age <= 25:
        ...
        model_object = model_young
    else:
        scaler_object = scaler_rest
        model_object = model_rest
    
    cols_to_scale = scaler_object['cols_to_scale']
    scaler = scaler_object['scaler']

    # Ensure income_level exists
    ...
    df = df[model_object.feature_names_in_]
    return df
